...
import '../storage/manager_storage.dart';
import '../storage/token_storage.dart';
import '../../features/pos/data/offline_sales_storage.dart';

class ManagerController extends ChangeNotifier {
  ManagerController({ManagerStorage? storage})
      : _storage = storage ?? const ManagerStorage();

  final ManagerStorage _storage;
  ...
  Future<void> saveManager({
    ...
    if (baseChanged) {
      await _clearManagerData();
    }

    notifyListeners();
  }

  Future<void> clearManager() async {
    _posUrl = null;
    _apiBaseUrl = null;
    _licenseKey = null;
  ...
  Future<void> _clearManagerData() async {
    ...
    const cacheKeys = [
      ...
      'pos_cached_categories',
      'pos_cached_payment_methods',
      'pos_cached_shipping_methods',
      'pos_cached_discounts',
      'pos_cached_config',
